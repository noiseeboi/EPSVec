...
class SteeredLlamaForCausalLM(LlamaForCausalLM):
    def __init__(self, config):
        super().__init__(config)
        self.steer_fn = lambda input_vector, hook: None #dummy function for now
        self.super_forward = super().forward
    
    def set_steer_fn(self, fn: Callable):
        self.steer_fn = fn        
    ...
    def forward(
        ...
        **model_kwargs: Any,
        ...
        return self.run_with_hooks(*model_args, **model_kwargs, fwd_hooks=[(lambda x: True, self.steer_fn)])
    ...
    def run_with_hooks(
        ...
        **model_kwargs: Any,
        ...
        with self.hooks(fwd_hooks, bwd_hooks, reset_hooks_end, clear_contexts) as hooked_model:
            return hooked_model.super_forward(*model_args, **model_kwargs)    # modified
############################################    