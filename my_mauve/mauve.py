...
from .mauve_utils import get_sentence_embedding_model, get_autoregressive_model_and_tokenizer, featurize_text_from_model, embed_text_from_sentence_embedder


class MauveEvaluator:
    def __init__(self, featurize_model_name = 'gpt2-large', device_id=0, use_float64=False):
        """
        ...
        :param ``use_float64``: If True, use float64 precision for the model. Default False.
        """
        
        if 'embedding' in featurize_model_name.lower():
            self.model = get_sentence_embedding_model(featurize_model_name, device_id)
            self.tokenizer = None
        else:
            self.model, self.tokenizer = get_autoregressive_model_and_tokenizer(featurize_model_name, device_id)
    ...
    def compute_mauve(self,
        ...
        * ``out.num_buckets`` is the quantization size and the shape of ``out.p_hist`` and ``out.q_hist``.
        ...
        p_features = self.get_features_from_input(p_text, max_text_length, name="p", verbose=verbose, batch_size=batch_size)
        q_features = self.get_features_from_input(q_text, max_text_length, name="q", verbose=verbose, batch_size=batch_size)
        if num_buckets == 'auto':
            # Heuristic: use num_clusters = num_generations / 10.
            num_buckets = max(2, int(round(max(p_features.shape[0], q_features.shape[0]) / 10)))
        ...
        t1 = time.time()
        p, q, p_smoothed, q_smoothed = self.cluster_feats(
        ...
        x_s, y_s = self.get_divergence_curve_for_multinomials(p_smoothed, q_smoothed, mixture_weights, mauve_scaling_factor).T
        ...
        fi_star = self.get_fronter_integral(p_smoothed, q_smoothed)
        ...
        if isinstance(divergence_curve, np.ndarray):
            divergence_curve = divergence_curve.tolist()

        to_return = SimpleNamespace(
            mauve=mauve_score, p_hist=p, q_hist=q, divergence_curve=divergence_curve, 
            frontier_integral=fi_score,
            mauve_star=mauve_star, frontier_integral_star=fi_star,
            num_buckets=num_buckets,
        )
        return self.NameSpaceToDict(to_return)
    @staticmethod
    def NameSpaceToDict(namespace):
        return {k: v for k, v in namespace.__dict__.items() if not k.startswith('_')}

    def get_features_from_input(self, texts, max_len, name, batch_size, verbose=False):
        if 'embedding' in self.featurize_model_name.lower():
            features = embed_text_from_sentence_embedder(self.model, texts, max_length=max_len, batch_size=batch_size, name=name, verbose=verbose).detach().cpu().numpy()
        else:
            features = featurize_text_from_model(self.model, self.tokenizer, texts, max_len, batch_size, name).detach().cpu().numpy()
        return features
